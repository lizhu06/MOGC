...
// [[Rcpp::export]]
void update_z_mogc(int N, int P, int K, Eigen::VectorXi& z,
...
void update_pi_k_mogc(int K, Eigen::VectorXi& nk, Eigen::VectorXd& alpha0, Eigen::VectorXd& pi_k){
  Eigen::VectorXd fake_pi_k(K);
  Eigen::VectorXd nk_d = nk.cast <double> ();
  Eigen::VectorXd alpha_new = alpha0 + nk_d; 
  for(int k = 0; k < K; k++){
    fake_pi_k[k] = R::rgamma(alpha_new[k], 1.0);
  }
  double sum_fake = fake_pi_k.sum();
  for(int k = 0; k < K; k++){
    pi_k[k] = fake_pi_k[k]/sum_fake;
  }
}

// [[Rcpp::export]]
Eigen::MatrixXd update_mu_mogc(int P, int P_dup, int K, Eigen::VectorXi& g_index, Eigen::VectorXi& l_index,
...
// [[Rcpp::export]]
void update_gamma_l_mogc(int P_dup, int N, int K, int m2, 
...
// [[Rcpp::export]]
void update_b_mogc(int P_dup, int N, int K,  
  Eigen::VectorXd& s2,
  Eigen::VectorXi& g_index,Eigen::VectorXi& l_index, Eigen::VectorXi& nk, Eigen::VectorXi& z, 
  ...
  for(int k = 0; k < K; k++){
    for(int pd = 0; pd < P_dup; pd++){
      ...
      if(gamma_l(k, l) * gamma_g(k,g) * gamma_j(k, pd) == 0){
        ...
        for(int i = 0; i < N; i++){
          if(z[i] == k + 1){
            sumy = sumy + Y(i, p)- mu_remove_this_b; //can be further optimized
          }
        }
...
// [[Rcpp::export]]
void update_sigma2_mogc(int P, int N, Eigen::VectorXi& z, Eigen::VectorXd& sigma2, 
...
// [[Rcpp::export]]
void update_pi_l_mogc(int K, int m2, 
...
// [[Rcpp::export]]
void update_pi_g_mogc(int P, int m2, int m1, int K, int BernoulliWeighted_int,
...
// [[Rcpp::export]]
void update_pi_j_mogc(int P_dup, int m1, int K, int BernoulliWeighted_int,
...
// [[Rcpp::export]]
void update_s2_mogc(int P_dup, int K, int T, Eigen::VectorXi& uni_types, 
...
// [[Rcpp::export]]
List MCMC_mogc(int seed, int burnInIter, int keepIter, int print_int, int N, 
  ...
  Eigen::VectorXi& z, Eigen::VectorXi& nk, Eigen::VectorXi& feature_dup_index,
  Eigen::VectorXd& alpha0, Eigen::MatrixXd& pi_j, Eigen::MatrixXd& pi_g, Eigen::VectorXd& pi_l,
  Eigen::VectorXd& sigma2, 
  ...
  for(int nsim = 0; nsim < NSIM; nsim++){
    ...
    if(!fix_mu){  
      update_gamma_l_mogc(P_dup, N, K, m2, g_index, l_index, nk, z, 
        feature_dup_index, pi_l, 
        sigma2, gamma_j, gamma_g, gamma_l, b, mu, Y);
      
      update_gamma_g_mogc(P_dup, N, K, m1, BernoulliWeighted_int,
        g_index, l_index, l_index_g, 
        nk, z, feature_dup_index, 
        sigma2, weight_for_Dg, gamma_j, gamma_g, gamma_l, 
        pi_g, b, mu, Y);
      
      update_gamma_j_mogc(P_dup, N, K, BernoulliWeighted_int,
        g_index, l_index, nk, z, feature_dup_index, 
        sigma2, weight_for_Tj, gamma_j, gamma_g, gamma_l, pi_j, b, mu, Y);

      /* update b */
      update_b_mogc(P_dup, N, K, s2, g_index, l_index,
        nk, z, feature_dup_index, types, 
        sigma2, gamma_j, gamma_g,gamma_l, b, mu, Y);

      /* update mu*/
      //mu = update_mu(P, P_dup, K, g_index, l_index, feature_dup_index,
      //  gamma_j, gamma_g, gamma_l, b);
    }
    ...
    /* update pi2*/
    update_pi_g_mogc(P, m2, m1, K, BernoulliWeighted_int, MH_ind,
    pi_g_prop_n, gl, l_index_g, weight_for_Dg, gamma_g, pi_g,
    pi_g_loglikeli);

    update_pi_j_mogc(P_dup, m1, K, BernoulliWeighted_int, MH_ind, 
    pi_j_prop_n, pg, g_index, weight_for_Tj, gamma_j, pi_j,
    pi_j_loglikeli);

    /* update s2*/
    if(!s2_fixed){
      update_s2_mogc(P, K, num_t, uni_types, types, feature_dup_index, s2, b);
    }
