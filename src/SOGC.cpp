...
// [[Rcpp::export]]
double update_z_sogc(int N, int P, int K, Eigen::VectorXi& z,
  ...
  for(int i = 0; i < N; i++){
    ...
    for(int k = 0; k < K; k++){
      ...
      for(int p = 0; p < P; p++){
        //if(!(mu.col(p).sum()==0.0)){
          ...
          if(false){
            cout << "i=" << i << "data_s[i,1]=" << Y(i,p) << ".\n";
            cout << "i=" << i << "logden=" << logden << ".\n";
            cout << "i=" << i << "sigma2=" << sigma2[p] << ".\n";
            cout << "i=" << i << "mu=" << mu(k, p) << ".\n";
          }
        //}
      }
    ...
    for(int k = 0 ; k < K; k++){
      if((k+1)== z[i]){
        logl_all = logl_all + logden_c[k]; //using previous z to calculate logl
      }
    }
    for(int k = 0; k < K; k++){
      if(ans[k] == 1){
...
void update_pi_c_sogc(int K, Eigen::VectorXi& nk, Eigen::VectorXd& alpha0, Eigen::VectorXd& pi_c){
  Eigen::VectorXd fake_pi_c(K);
  Eigen::VectorXd nk_d = nk.cast <double> ();
  Eigen::VectorXd alpha_new = alpha0 + nk_d; 
  for(int k = 0; k < K; k++){
    fake_pi_c[k] = R::rgamma(alpha_new[k], 1.0);
  }
  double sum_fake = fake_pi_c.sum();
  for(int k = 0; k < K; k++){
    pi_c[k] = fake_pi_c[k]/sum_fake;
  }
}

// [[Rcpp::export]]
Eigen::MatrixXd update_mu_sogc(int P, int P_dup, int K, Eigen::VectorXi& g_index,
...
// [[Rcpp::export]]
void update_gamma_g_sogc(int P_dup, int N, int K, int G, 
...
// [[Rcpp::export]]
void update_gamma_j_sogc(int P_dup, int N, int K, int BernoulliWeighted_int, 
...
// [[Rcpp::export]]
void update_b_sogc(int P_dup, int N, int K,  
  Eigen::VectorXd& s2,
  Eigen::VectorXi& g_index, Eigen::VectorXi& nk, Eigen::VectorXi& z, 
...
// [[Rcpp::export]]
void update_sigma2_sogc(int P, int N, Eigen::VectorXi& z, Eigen::VectorXd& sigma2, 
...
// [[Rcpp::export]]
void update_pi_g_sogc(int K, int G, 
...
// [[Rcpp::export]]
void update_pi_j_sogc(int P_dup, int G, int K, int BernoulliWeighted_int,
...
// [[Rcpp::export]]
void update_s2_sogc(int P_dup, int K, int T, Eigen::VectorXi& uni_types, 
...
// [[Rcpp::export]]
List MCMC_sogc(int seed, int burnInIter, int keepIter, int print_int, 
  ...
  Eigen::VectorXi& feature_dup_index, Eigen::VectorXi& U1_rowsum, 
  Eigen::VectorXd& alpha0, Eigen::VectorXd& pi_g, Eigen::VectorXd& sigma2, 
  Eigen::VectorXd& pi_c, Eigen::VectorXd& weight_for_Rj,  
  Eigen::MatrixXi& gamma_g, Eigen::MatrixXi& gamma_j,
  Eigen::MatrixXd& pi_j, Eigen::MatrixXd& b, Eigen::MatrixXd& mu, 
  Eigen::MatrixXd& Y, bool debug, bool fix_z, bool fix_mu,
  Eigen::MatrixXd& pi_j_loglikeli, bool s2_fixed){
  ...
  Eigen::VectorXd total_logl(NSIM); 
  ...
  for(int nsim = 0; nsim < NSIM; nsim++){
    ...
    if(!fix_mu){
      /* update gamma_g*/ 
      update_gamma_g_sogc(P_dup, N, K, G, g_index, nk, 
        z, feature_dup_index, U1_rowsum, pi_g, sigma2, gamma_g, gamma_j, b, mu, Y);

      /* update gamma_j */
      update_gamma_j_sogc(P_dup, N, K, BernoulliWeighted_int, 
        g_index, pg, nk, z, feature_dup_index, U1_rowsum,
        sigma2, weight_for_Rj, 
        gamma_g, gamma_j, pi_j, b, mu, Y);

      /* update b */
      update_b_sogc(P_dup, N, K, s2, g_index, 
        nk, z,  feature_dup_index, U1_rowsum, types, 
        sigma2, gamma_g, gamma_j, b, mu, Y);

      /* update mu*/
      //mu = update_mu(P, P_dup, K, g_index, feature_dup_index, gamma_g, gamma_j, b);
    }
    ...
    /* update pi_j*/
    update_pi_j_sogc(P_dup, G, K, BernoulliWeighted_int, MH_ind, pi_j_prop_n,
      pg, g_index, weight_for_Rj, gamma_j, pi_j, 
      pi_j_loglikeli);

    /* update s2*/
    if(!s2_fixed){
      update_s2_sogc(P_dup, K, num_t, uni_types, types, feature_dup_index, s2, b);
    }
    
    /* update cluster index */
    if(!fix_z){
      total_logl[nsim] = update_z_sogc(N, P, K, z, pi_c, sigma2, mu, Y, Prob_Assign, debug);
    }

    /* get nk */
    nk = get_nk_sogc(N, K, z);
  ...
  return List::create(Rcpp::Named("Z_store") = Z_store,
                            ...
                            Rcpp::Named("total_logl") = total_logl,
