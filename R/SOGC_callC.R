#' Bayesian Gaussian mixture model incorporating Single-layer Overlapping Group structure (SOGC)
#'
#' This function applies SOGC clustering model to a feature matrix
#'
...
#'
#' @examples
SOGC <- function(Y, U1, K, center=FALSE,
  ...
  pi_j_prop_n=10, s2=NULL,
  types=NULL, report_dup_mean=FALSE){
  ...
  G <- ncol(U1)
  pg <- apply(U1, 2, sum)
  g_index <- unlist(lapply(1:P, function(j) which(U1[j,]==1))) # length as p_dup
  ...
  # weight for Bernoulli prior (of the same dimension as duplicated feature)
  if(BernoulliWeighted){
    weight_for_Rj <- unlist(lapply(1:length(U1_rowsum), function(j) rep(1/U1_rowsum[j], U1_rowsum[j])))
  }else{
  ...
  mu_dup <- gamma_g_rep*gamma_j*b # same order as the row of U1
  ...
  ## MCMC
  #Rcpp::sourceCpp(paste0(cppfile))
  res <- MCMC_sogc(seed, burnInIter, keepIter, print_int, N, P, P_dup, G,
    K, BernoulliWeighted_int, MH_ind,
    s2, pi_j_prop_n,
    uni_types, types,
    g_index, pg, z, nk, feature_dup_index, U1_rowsum,
    alpha0, pi_g, sigma2,
    pi_c, weight_for_Rj,
    gamma_g, gamma_j,
    pi_j, b, mu, Y, debug, fix_z, fix_mu,
    pi_j_loglikeli, s2_fixed)
  ...
  repeat{
    ...
    res <- MCMC_sogc(seed, burnInIter=0, keepIter, print_int,
      N, P, P_dup, G,
      K, BernoulliWeighted_int, MH_ind,
      s2=res$S2_store[, keepIter], pi_j_prop_n,
      uni_types, types,
      g_index, pg, z=res$Z_store[keepIter,],
      nk=res$NK_store[keepIter,], feature_dup_index, U1_rowsum,
      alpha0, pi_g=res$pi_g_store[keepIter,],
      sigma2=res$SIGMA2_store[keepIter,],
      pi_c=res$PI_C_store[keepIter,], weight_for_Rj,
