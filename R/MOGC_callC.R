#' Bayesian Gaussian mixture model incorporating Multi-layer Overlapping Group structure (MOGC)
#'
...
#'
#' @examples
MOGC <- function(Y, U1, U2, K, center=FALSE,
  ...
  BernoulliWeighted=TRUE, MH_ind=0, pi_g_prop_n=10,
  pi_j_prop_n=10, 
  ...
  res <- MCMC_mogc(seed, burnInIter, keepIter, print_int, N, P, P_dup, m1, m2, K,
    BernoulliWeighted_int, MH_ind,
    s2, pi_g_prop_n, pi_j_prop_n,
    uni_types, types,
    g_index, l_index, l_index_g,
    pg, pl, gl, z, nk, feature_dup_index,
    alpha0, pi_j, pi_g, pi_l, sigma2, pi_k, weight_for_Dg, weight_for_Tj,
  ...
  repeat{
    ...
    res <- MCMC_mogc(seed, burnInIter=0, keepIter, print_int, N, P,
      P_dup, m1, m2, K,
      BernoulliWeighted_int, MH_ind,
      s2=res$S2_store[, keepIter], pi_g_prop_n, pi_j_prop_n,
      uni_types, types,
      g_index, l_index, l_index_g,
      pg, pl, gl, z=res$Z_store[keepIter,], nk=res$NK_store[keepIter,],
      feature_dup_index,
      alpha0, pi_j=matrix(res$PI_J_store[keepIter,], K, m1, byrow=TRUE),
  ...
  if(report_dup_mean){
    ...
    res$GAMMA_L_store <- GAMMA_L_store_o
    res$GAMMA_G_store <- GAMMA_G_store_o
    res$GAMMA_J_store <- GAMMA_J_store_o
    res$B_store <- B_store_o
    res$PI_G_store <- PI_G_store_o
